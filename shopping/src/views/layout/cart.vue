      ...
      <div class="all-total">
        <div class="price">
          ...
          <span
            >¥
            <i class="totalPrice">{{
              $store.getters["cart/selPrice"]
            }}</i></span
          >
        </div>
        <div v-if="!isEdit" class="goPay" @click="goOrder">
          结算({{ $store.getters["cart/selCount"] }})
        </div>
        <div v-else @click="handleDel" class="delete">删除({{$store.getters["cart/selCount"]}})</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters("cart", ["cartTotal", "isAllChecked","selCartList"]),
  ...
  methods: {
    ...mapMutations("cart", ["isOneChecked"]),
    ...mapActions("cart", ["changeAsy"]),
    toggleCheck(goodsId) {
      this.isOneChecked(goodsId);
    },
    toggleAllCheck() {
